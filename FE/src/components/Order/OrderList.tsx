...
export default function OrderList() {
    ...
    const urlCurrentPage = useAppSelector(
        (state: RootState) => state.order.urlCurrentPage
    );
    const urlNextPage = useAppSelector(
        (state: RootState) => state.order.urlNextPage
    );
    const urlPrePage = useAppSelector(
        (state: RootState) => state.order.urlPrePage
    );
    const totalPage = useAppSelector(
        (state: RootState) => state.order.totalPage
    );
    const currentPage = useAppSelector(
        (state: RootState) => state.order.currentPage
    );

    const [lastUrlCurrentPage, setLastUrlCurrentPage] = useLocalStore({
        key: "LastUrlCurrentPage",
        initialValue: "",
    });
    const [lastDayFrom, setLastDayFrom] = useLocalStore({
        key: "lastDayFrom",
        initialValue: "",
    });
    const [lastDayTo, setLastDayTo] = useLocalStore({
        key: "lastDayTo",
        initialValue: "",
    });
            ...
            <table className="table">
                ...
                <tbody className="body-table-detail-box">
                    {orderlist ? (
                        orderlist.map((order) => {
                            return (
                                <ProductInfor product={order} key={+order.id} />
                            );
                        })
                    ) : (
                        <></>
